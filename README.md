...
| `data/sssc.csv` | 歷史資料 |
| `.github/workflows/track.yml` | GitHub Actions 排程，自動持續抓取資料 |
| `.github/workflows/chart.yml` | GitHub Actions 排程，每天中午/晚上各出一張圖 |
...
`chart.yml` 每天**中午 12:00** 與**晚上 21:00**（台北時間）各跑一次，畫出當下的
趨勢圖並存進 `data/`，檔名如 `data/chart_2026-07-02_noon.png`、
`data/chart_2026-07-02_evening.png`，每天累積兩張，可直接在 GitHub 上點開看。
（想改時間就改 `chart.yml` 裡的 cron。）
